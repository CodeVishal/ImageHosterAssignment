...
public class UserController {
    ...
    This method calls the business logic and after the user record is persisted in the database,
     directs to login page if password field contains  at least 1 alphabet, 1 number & 1 special character

     if not then it  will display error messge in user/registration page
    */
    ...
    public String registerUser(User user ,Model model) {
        ...
        if(user.getPassword().matches("(?=.*[A-Za-z])(?=.*[0-9])(?=.*[~!@#$%^&*_+=-?><])[A-Za-z0-9!~!@#$%^&*()_+=-?><]*")) {
